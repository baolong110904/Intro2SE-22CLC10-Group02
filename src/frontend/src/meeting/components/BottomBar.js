...
import { OutlinedButton } from "../../components/videosdk/buttons/OutlinedButton"
import useIsTab from "../../hooks/useIsTab"
import useIsMobile from "../../hooks/useIsMobile"
import { MobileIconButton } from "../../components/videosdk/buttons/MobileIconButton"
                                ...
                                {mics.map(({ deviceId, label }, index) => (
                                  <div
                                    className={`px-3 py-1 my-1 pl-6 text-white text-left ${
                                      deviceId === selectedMic.id && "bg-gray-150"
                                    }`}
                                  >
                                    <button
  ...
  const ScreenShareBTN = ({ isMobile, isTab }) => {
    ...
    return isMobile || isTab ? (
      <MobileIconButton
        ...
        disabled={
          presenterId ? (localScreenShareOn ? false : true) : isMobile ? true : false
        }
      />
    ) : (
      <OutlinedButton
