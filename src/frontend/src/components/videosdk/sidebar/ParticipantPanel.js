...
function ParticipantListItem({ participantId, raisedHand }) {
  ...
  console.log("ParticipantListItem", participantId, raisedHand)
  console.log("ParticipantListItem", micOn, webcamOn, displayName, isLocal)
    ...
    >
      <div className="flex flex-col flex-1" style={{ height: panelHeight - 100 }}>
        {[...participants.keys()].map((participantId, index) => {
          ...
          return (
            <ParticipantListItem participantId={peerId} raisedHand={raisedHand} />
          )
        })}
      </div>
    </div>
