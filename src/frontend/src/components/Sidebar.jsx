  ...
  return (
    ...
    >
      <div className="flex flex-col justify-center items-center gap-8 w-full">
        {isExpanded ? (
          <div id="logo-box">
            <h1 className={`font-bold text-4xl ${isDarkMode ? 'text-red-600' : 'text-blue-600'}`}>
              G2 <span className={`italic ${isDarkMode ? 'text-yellow-500' : 'text-green-500'}`}>LL</span>
            </h1>
          </div>
        ) : (
          <div className="flex justify-center items-center">
            <h1 className={`font-bold text-3xl ${isDarkMode ? 'text-red-500' : 'text-blue-500'}`}>G</h1>
            <span className={`font-bold text-3xl ${isDarkMode ? 'text-yellow-500' : 'text-green-500'}`}>2</span>
          </div>
        )}
          ...
          {navItems.map((item, index) => (
            ...
            >
              <motion.div
                ...
                whileTap={{ scale: 0.95 }}
              >
                <div className={`flex items-center ${isDarkMode ? 'bg-yellow-300 text-black' : 'bg-blue-300 text-black'} p-2 rounded-full`}>
