...
export default function ThemeToggle() {
  ...
  return (
    <div>
      ...
      />
      <label
        className={`flex items-center justify-center cursor-pointer w-8 h-8 bg-slate-100 hover:bg-slate-200 ${currentTheme === 'dark' ? 'bg-blue-900' : 'bg-slate-100'} rounded-full`}
        htmlFor="light-switch"
      >
        <svg className={`w-4 h-4 ${currentTheme === 'dark' ? 'hidden' : 'block'}`} width="16" height="16" xmlns="http://www.w3.org/2000/svg">
          <path
        ...
        <svg className={`w-4 h-4 ${currentTheme === 'dark' ? 'block' : 'hidden'}`} width="16" height="16" xmlns="http://www.w3.org/2000/svg">
          <path
            className="fill-current text-slate-400"
