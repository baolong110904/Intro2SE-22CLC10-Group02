...
const Navbar = ({ userName, isDarkMode, toggleTheme }) => {
  return (
    <header className={`p-4 flex justify-between items-center ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-blue-800 text-white'}`}>
      ...
      <nav className="flex items-center">
        ...
        <button onClick={toggleTheme} className={`mr-4 ${isDarkMode ? 'text-white' : 'text-black'}`}>
        ...
        <Menu as="div" className="relative">
          ...
          <Menu.Items className="absolute right-0 mt-2 w-48 bg-white text-black rounded-md shadow-lg dark:bg-gray-800 dark:text-white">
            <Menu.Item>
              {({ active }) => (
                <Link to="/profile" className={`${active ? 'bg-gray-100 dark:bg-gray-700' : ''} block px-4 py-2`}>
            ...
            <Menu.Item>
              {({ active }) => (
                <Link to="/change-password" className={`${active ? 'bg-gray-100 dark:bg-gray-700' : ''} block px-4 py-2`}>
            ...
            <Menu.Item>
              {({ active }) => (
                <Link to="/" className={`${active ? 'bg-gray-100 dark:bg-gray-700' : ''} block px-4 py-2`}>
