  ...
  return (
    <div className="p-4 bg-gray-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        ...
        <Card
          title="Average Score"
          value={<CountUp end={8.8} duration={2.5} decimals={1} />}
        />
        <Card
          ...
          <div className="overflow-hidden rounded-full h-3 bg-gray-200 flex">
            {deviceData.map((item, index) => (
              <div
                ...
                style={{ width: `${item.size}%`, transition: "width 1s ease-out" }}
              ></div>
            ))}
          </div>
        </div>
          ...
          {deviceData.map((item, index) => (
            <div key={index} className="text-center">
              <div className="text-sm mb-2">
                <span
                  className={`inline-block w-2 h-2 rounded-full mr-1 ${item.color}`}
                ></span>
